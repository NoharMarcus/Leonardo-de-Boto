...
def create_manual_instructions(d, painting_points):
    ...
    for point in painting_points:
        ...
        alpha = calculator.Rad2Degree(calculator.alpha)
        beta = calculator.Rad2Degree(calculator.beta)
...
def send_alpha_beta_vectors(d, alpha_vec, beta_vec, de_boto_comm):
    ...
    commands_vector = tcp_send.prepare_data_to_send(alpha_vec, beta_vec)
...
def activate_De_Boto(img_path):
    ...
    for vector in lines:
        ...
        alpha_vec.insert(1, np.pi*de_boto_comm.lower_code/180)
        beta_vec.insert(1, np.pi*de_boto_comm.lower_code/180)
        alpha_vec.append(np.pi*de_boto_comm.lift_code/180)
        beta_vec.append(np.pi*de_boto_comm.lift_code/180)
