...
#constants
DE_boto_photo ="./images/Leonardo_De_Boto2.png"
...
def new_canvas(filename):
    ...
    draw_without_hatch_button.grid(column = 2, row = 1 )
    ...
    #delete this
    imtoshow = im_from_lines.create_img_from_lines(line_without_hatch)
    imtoshow.show()
    ############################################################################
    ...
    label_without_hatch.grid(column = 2, row = 0)

    #adding debug checkbox
    debugcheckbox = tk.Checkbutton(root,text = "Debug mode" , variable = debug , onvalue = True, offvalue = False)
    debugcheckbox.grid(row = 1 , column = 1)
    
#drawing function
def send_drawing_to_function(filename,draw_with_hatch):
    linedraw.draw_hatch = draw_with_hatch
    print(debug)
    roboto_main.debug = debug
    ...
    root2.after(2000,roboto_main.activate_De_Boto(filename))
    root2.mainloop()
...
try:
    ...
    logo_label = tk.Label()
    print("picture could not be loaded check path")
